...
export default class Home extends Component {
  ...
  getQuestion = () => {
    ...
    getQuestion().then(response => {
      ...
      if (response.errors || response.error) {
        ...
        setTimeout(() => Alert.alert(response.errors ? response.errors : response.error), 500);
      } else {
        self.setState({ loading: false, question: this.loadOptionImages(response), answerSubmitted: response.is_answered }, () => {
          if (response.is_answered) {
            self.updateItem(self.getRightOptionIndex(response));
          }
        });
        console.log('+++getRightOptionIndex+++>>>', self.getRightOptionIndex(response));
  ...
  onReload = async () => {
    const self = this;
    this.setState({ isAnswerSelected: false });
    getUser().then(user => self.setState({ user: user }));
    this.getQuestion();
  }
  ...
  onSubmitAnswer = async () => {
    ...
    if (isAnswerSelected) {
      ...
      if (answerSubmitted) {
        Alert.alert(
          'Are you sure?',
          'Reseting will remove your answer and refresh the questions.',
          [
            {
              text: 'Cancel',
            ...
            {
              text: 'OK', onPress: () => this.resetQuestion()
            }
          ]);
      }
    }
  ...
  resetQuestion = () => {
    ...
    this.setState({ isAnswerSelected: false, answerSubmitted: false, question });
  }
  ...
  render() {
    ...
    console.log('~~~~~ question question_image ~~~~>>>', question);
    return (
      <MainCon
          ...
          <PageWidth paddingBottom={50}>
            {
              ...
              <AlignCenter
                ...
                <ImageLoad
                  ...
                  resizeMode='contain'
                //onLoaded={()=> console.log('0000000000>')}
                />

                {/*<ProgressiveImage
                  source={{uri: question.question_image}}
                  thumbnail={PLACEHOLDER_BANNER}
                ...
                {isValid(question) && question.question_options.map((item, index) => {
                  return (
                    <AnswerItem
                      ...
                      showFeedback={(answerSubmitted) && item.isSelected ? true : false}
              ...
              </CardInfoTop>
              {
                <AlignCenter marginTop={-20} marginBottom={20}>
                  <CustomBtn
                    ...
                    btnText={!answerSubmitted ? 'SUBMIT' : 'RESET QUESTION'}
                    textColor={WHITE}
                    fontSize={16}
