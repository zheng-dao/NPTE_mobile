...
export default class QuestionComponent extends Component {
  ...
  componentDidMount() {
    ...
    getQuestion().then(response => {
      ...
      if(response.errors || response.error){
        ...
        self.setState({loading: false, question: this.loadOptionImages(response), answerSubmitted: true}, ()=>{
  ...
  onReload = async () => {
    const self = this;
    getUser().then(user => self.setState({ user: user }));
    const question = await self.props.question; //self.props.navigation.state.params.question;
    self.setState({ question: this.loadOptionImages(question) }, () => {
      if (question.is_answered) {
        self.updateItem(self.getRightOptionIndex(question));
      }
    });
              ...
              </CardInfoTop>
              {
                ...
                <AlignCenter marginTop={-20} marginBottom={20}>
                  <CustomBtn
                    ...
                    borderRadius={7}
                    boxShadow={true}
                    isButtonDisable={!isAnswerSelected}
